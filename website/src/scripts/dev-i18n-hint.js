...
const LOCALE_SEGMENTS = new Set(["zh-Hans", "en", "ja", "es", "ko"]);
...
function warnIfDevLocaleMismatch() {
  ...
  console.warn(
    `[Docusaurus i18n] 开发模式一次只编译一种语言，当前为「${i18n.currentLocale}」，而地址前缀是「${implied}」。这样会出现 404、图片或视频加载失败。请使用 npm run start（中文）或 npm run start:${implied}；若要像线上一样测全部语言，请用 npm run preview。`,
  );
}
