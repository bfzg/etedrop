...
export function useSharePage(deviceId: string, shareCode: string) {
  ...
  const fileInfoRef = useRef<{ fileName: string; fileSize: number; hasPassword: boolean } | null>(null)
        ...
        switch (m.type) {
          ...
          case 'answer':
            if (pcRef.current && m.data) {
              pcRef.current.setRemoteDescription(new RTCSessionDescription(m.data))
            }
            break
          case 'ice-candidate':
            if (pcRef.current && m.data) {
              pcRef.current.addIceCandidate(new RTCIceCandidate(m.data)).catch(() => {})
            }
            break
          case 'err':
            setStatusState(
              'error',
              m.code === 'OFFLINE' ? '分享者设备离线，请稍后再试' : m.msg || '连接异常',
            )
    ...
    (ws: WebSocket) => {
      ...
      dc.onmessage = (ev) => {
        if (typeof ev.data === 'string') {
          ...
          const received =
            chunksRef.current.reduce((acc, c) => acc + c.byteLength, 0)
