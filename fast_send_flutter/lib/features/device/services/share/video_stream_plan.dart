...
class VideoStreamPlanner {
  ...
  static VideoStreamPlan plan(VideoStreamProbe probe) {
    ...
    return RemuxVideoStreamPlan(
      ...
      codecParts: parts,
      // remux 模式下 ffmpeg 是按磁盘 IO 速度 dump 的：1080p / 720p 视频也能在
      // 几秒内把 100MB+ 灌进 DC。即使我们做了 bufferedAmount 背压，flutter_webrtc
      // 的 `bufferedAmount` 在 Android 上**不一定**真实反映底层 SCTP 队列，
      // 一旦实际网络/中继带不动，就会触发 ICE consent check 超时 → PC Disconnected
      // → 整个会话被对端拆掉。
      //
      // `-re` 让 ffmpeg 按媒体时间锁读速（约等于源码率），换来稳定可靠的播放体验。
      // 若 4K 也叠加 transcode 用 -re，CPU 会闲等，不划算；transcode 那边继续 false。
